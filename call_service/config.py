...
Default posture: Understanding, Supportive, Collaborative.
...
Friendly, Calm, Professional, Non judgmental.
...
def parse_customer_data(body: dict) -> dict:
    ...
    last_comm = communications[-1] if communications else {}

    return {
        "CUSTOMER_FIRST_NAME": customer.get("first_name", ""),
        "CUSTOMER_LAST_NAME": customer.get("last_name", ""),
        "CURRENT_BALANCE": first_debt.get("current_balance", ""),
        "DAYS_PAST_DUE": first_debt.get("days_past_due", ""),
        "PROFILE_TYPE": customer.get("account_status", ""),
        ...
        "LAST_CONTACT_DATE": last_comm.get("timestamp", ""),
        "LAST_CONTACT_OUTCOME": last_comm.get("outcome", ""),
        "CUSTOMER_NOTES": customer.get("notes", ""),
    }


def build_prompt(userdata: dict) -> str:
    prompt = MASTER_PROMPT
    for var in TEMPLATE_VARIABLES:
        prompt = prompt.replace("{{" + var + "}}", str(userdata.get(var, "")))
    return prompt.strip()
